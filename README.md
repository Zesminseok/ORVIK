...
This release is a **240-day demo release**. The demo period starts on first launch. When the demo period ends, BRIDGE+ still launches and continues in limited mode with basic Pro DJ Link to TCNet output for timecode, faders, track metadata, and artwork; extended features are disabled.
...
이 릴리스는 **240일 데모 릴리스**입니다. 데모 기간은 첫 실행일부터 시작됩니다. 데모 기간이 종료되어도 BRIDGE+ 실행 자체는 차단되지 않으며, 제한 모드에서 타임코드, 페이더, 곡 메타데이터, 앨범아트의 기본 Pro DJ Link → TCNet 출력만 계속 작동하고 확장 기능은 비활성화됩니다.
