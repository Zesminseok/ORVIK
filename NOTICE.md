...
BRIDGE+ desktop binaries are proprietary demo binaries governed by `BINARY_LICENSE.md` and by any terms shown in the application or release materials. Demo expiration does not block application launch; after expiration, BRIDGE+ continues only the basic Pro DJ Link to TCNet output described in the binary license, while extended features are disabled. No permission is granted to copy, modify, redistribute, mirror, sell, sublicense, or reverse engineer BRIDGE+ except where applicable law cannot restrict those rights.
...
BRIDGE+ 데스크톱 바이너리는 `BINARY_LICENSE.md` 및 앱/릴리스 자료에 표시된 조건의 적용을 받는 독점 데모 바이너리입니다. 데모 기간이 종료되어도 앱 실행 자체는 차단되지 않으며, 이후에는 바이너리 라이선스에 설명된 기본 Pro DJ Link → TCNet 출력만 계속 작동하고 확장 기능은 비활성화됩니다. 관련 법률상 제한할 수 없는 경우를 제외하고 BRIDGE+의 복사, 수정, 재배포, 미러링, 판매, 재라이선스, 리버스 엔지니어링은 허용되지 않습니다.
